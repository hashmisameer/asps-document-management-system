 ...
 * sample, and recorded against the TYPE. Nothing here touches a document, its
 * own placements, or a stored file: saving a template stamps nothing, and
 * there is no action that applies one to the documents already there. The
 * next change stamps NEW uploads from it.
 */
...
/**
 * Replaces a type's template with the set given.
 *
 ...
 * stamper uses, said now rather than at the first upload; or the boxes were
...
export async function save(
  ...
  // two different documents would describe neither.
  ...
  await documentTypePlacementRepository.replaceForType(
    ...
    { documentId: input.sampleDocumentId, pageCount: input.samplePageCount },
    actor.userId,
  )
  ...
  await audit.record({
    ...
    metadata: {
      ...
      samplePageCount: input.samplePageCount,
    },
  })
