...
export const login: RequestHandler = async (req, res) => {
  ...
  const result = await authService.login(input, contextOf(req))
...
export const logout: RequestHandler = async (req, res) => {
  ...
  if (user && sessionId !== undefined) {
    await authService.logout(sessionId, user.userId, contextOf(req))
  }
...
export const changePassword: RequestHandler = async (req, res) => {
  ...
  const result = await authService.changePassword(req.user, input, contextOf(req))
