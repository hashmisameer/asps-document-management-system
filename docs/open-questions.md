...
| Q6 | May Viewers **download** documents, or preview only? | **DECIDED** | **They may download.** Decided 2026-09-19 by Sameer Hashmi for the office: Management (the Viewer role) sees, downloads and prints everything in the DMS and changes nothing. `DOCUMENT_DOWNLOAD` was added to `VIEWER_PERMISSIONS`; that opened the document download route and the employee-file print, the only two surfaces a Viewer lacked. The office knows the employee file is one PDF with the Aadhaar and PAN scans inside it; every such document could already be previewed. |
